...
// 70
  ...
  // return <div>
  //   <label>Среднее арифмитическое: {get_average(notes)}</label>
  //   <div>
  //     {result}
  //   </div>
  // </div>;
...
//   </div>; 
//  } 
//isEdit и data могут быть стейтами или переменными, а elem может быть переменной.
...
//   return (
//     <input type="number" value={temp} onChange={handleTempChange} />
//   );
// };

// // Компонент Verdict
// const Verdict = ({ temp }) => {
//   const getVerdict = (temp) => {
//     if (temp < 0) {
...
//     setNums(prevNums => [...prevNums, newValue]); // добавляем элемент в массив
//     setSum(prevNums => getSum([...prevNums, newValue])); // вычисляем сумму заново
//   }

//   const sum = getSum(nums); // пересчитываем сумму
